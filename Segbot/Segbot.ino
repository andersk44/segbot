...
void setup()
{
  ...
  Ti = 200; // Tidskonstant för integrering 200
  p1 = K*Td/h;
  p2 = K*h/Ti;
  // End ny kod
}
...
void update_motor_speed()
{
  ...
  if (engage == true)
  {
    ...
    else 
    {
      ...
      u = P + I + D; // P-reglering + integrering + derivering
      output = (u * -1000);
      I = I + p2 * angle; // Integrering
      angle_old = angle;
      motor_out = map(output, -80, 80, -gain_val, gain_val); // map the angle. Ändring av insignal från 250 till 70
      if(motor_out > 0.7 * gain_val)
      {
        motor_out = gain_val;
      }
      if(motor_out < 0.7 * -gain_val)
